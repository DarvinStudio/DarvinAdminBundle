...
field_blacklist:
    ...
    # Прототип
    some_role: [] # Список полей, которые будут скрыты для пользователей с определенной ролью
field_whitelist:

    # Прототип
    some_role: [] # Список полей, которые будут показаны пользователям с определенной ролью, остальные будут скрыты
